...
ordonnee_ranges = {'NO': [0, 15], 'NOX': [0, 40], 'O3': [10, 90], 'PM10': [0, 25], 'NO2': [0, 25]}

# Boucle à travers les polluants pour créer les graphiques
for polluant in polluants:
    # Filtrer les données pour le polluant et le département du TARN
    filtered_data = bdd1[(bdd1['nom_dept'] == 'TARN') & (bdd1['nom_poll'] == polluant)]
    ...
    fig = px.scatter(
        ...
        title=f"Évolution de la pollution {polluant} dans le TARN",
    ...
    fig.update_layout(
        title_text=f"Évolution de la pollution {polluant} dans le TARN",
...
# Plages des ordonnées pour chaque polluant
ordonnee_ranges = {'NO': [0, 15], 'NOX': [0, 30], 'O3': [45, 70], 'PM10': [10, 20], 'NO2': [0, 20]}

# Boucle à travers les polluants pour créer les graphiques
for polluant in polluants:
    # Filtrer les données pour le polluant et le département TARN
    filtered_data = bdd1[(bdd1['nom_dept'] == 'TARN') & (bdd1['nom_poll'] == polluant)]
    filtered_data = filtered_data.sort_values(by='date_debut')
