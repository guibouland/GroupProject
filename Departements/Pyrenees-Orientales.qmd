...
ordonnee_ranges = {'NO': [0,70], 'NOX': [0, 160], 'O3': [15, 95], 'PM10': [0, 35], 'NO2': [0, 60]}

# Boucle à travers les polluants pour créer les graphiques
for polluant in polluants:
    # Filtrer les données pour le polluant et le département des PYRENEES-ORIENTALES
    filtered_data = bdd1[(bdd1['nom_dept'] == 'PYRENEES-ORIENTALES') & (bdd1['nom_poll'] == polluant)]
    ...
    fig = px.scatter(
        ...
        title=f"Évolution de la pollution {polluant} dans les PYRENEES-ORIENTALES",
    ...
    fig.update_layout(
        title_text=f"Évolution de la pollution {polluant} dans les PYRENEES-ORIENTALES",
        ...
        paper_bgcolor='rgba(32,32,32,50)',  # Couleur de fond
        plot_bgcolor='rgba(255, 244, 238, 88)',   # Couleur du graphique
...
Evolution des 5 types de polluants au cours des 5 dernières années :
```{python}
import pandas as pd
import plotly.express as px
import plotly.graph_objects as go
...
# Plages des ordonnées pour chaque polluant
ordonnee_ranges = {'NO': [0, 85], 'NOX': [0, 180], 'O3': [45, 90], 'PM10': [0, 30], 'NO2': [0, 70]}

# Boucle à travers les polluants pour créer les graphiques
for polluant in polluants:
    # Filtrer les données pour le polluant et le département PYRENEES-ORIENTALES
    filtered_data = bdd1[(bdd1['nom_dept'] == 'PYRENEES-ORIENTALES') & (bdd1['nom_poll'] == polluant)]
    filtered_data = filtered_data.sort_values(by='date_debut')
