...
polluants_list = ['PM10', 'O3', 'NOX', 'NO2', 'PM2.5', 'NO']
...
def concentration(date, format, mapping, titre, temps):
    ...
    resultat[date] = resultat[date].replace(mapping)
...
graphique_axe(fig, 'Vitesse du vent et émissions de NOx(mars 2023)', 'Vitesse (km.h⁻¹)')
...
def tableau(mesure, titre, gaz, axe):
    ...
    # Ajouter la trace pour le vecteur vent_moy sur le deuxième axe Y
    trace_temp_moy = go.Scatter(x=temp_moy["Date"], y=temp_moy["valeur"], mode='lines', line=dict(width=2, color='black'), name=titre, yaxis='y2')

    # Créer de la figure
    fig = go.Figure([trace1, trace_temp_moy])

    graphique_axe(fig, f'{titre} et {gaz}', axe)

    # Afficher le graphique
    fig.show()

tableau('Température', 'Température', 'ozone', 'Température (°C)')
```
<br>
On ne remarque pas de lien particulier entre la température et la quantité d'ozone dans l'air.

## Lien entre la pluie et le taux de pollution

```{python}
tableau("Précipitations dans les 24 dernières heures", 'Précipitations', "oxydes d'azote", 'Précipitations (mm)')
```
<br>
Il faut que je rajoute les oxydes d'azote

## Lien entre la pression et le taux de pollution

```{python}
tableau("Pression station", 'Pression', "oxydes d'azote", 'Pression (Pa)')
```
<br>
idem